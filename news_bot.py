...
۲. تاثیر دقیق روی **اقتصاد جهانی** و **قیمت طلا** را توضیح بده
۳. مشخص کن که این خبر برای طلا صعودی است، نزولی است یا خنثی — با دلیل

سپس یک **تحلیل جامع** از کل اخبار بنویس که شامل:
- وضعیت کلی بازار طلا بر اساس اخبار
- جهت احتمالی قیمت طلا با استدلال قوی
...
فقط JSON برگردان، هیچ توضیح اضافه‌ای نه:
{{
  "selected": [
    {{
      "index": 1,
      "title_fa": "ترجمه فارسی",
      "impact": "تاثیر روی اقتصاد جهانی و طلا",
      "direction": "صعودی/نزولی/خنثی — دلیل"
    }}
  ],
  "analysis": "تحلیل جامع در ۶ تا ۱۰ جمله..."
}}
"""
    response = call_ai(prompt, max_tokens=5000)
    if not response:
        return articles[:10], ''
...
برای **هر رویداد**:
۱. جایگاه این داده در اقتصاد (آمریکا/اروپا/چین) را توضیح بده — این نرخ چه چیزی را نشان می‌دهد؟
۲. تاثیر مستقیم این داده بر قیمت طلا را تحلیل کن (با دلیل)
۳. اگر مقدار actual موجود است، آن را با forecast مقایسه کن و تفسیر کن که بهتر یا بدتر از پیش‌بینی بوده

در آخر یک **جمع‌بندی کلی** بنویس از وضعیت داده‌های اقتصادی امروز برای معامله‌گر طلا.
...
{{
  "events": [
    {{
      "title": "اسم رویداد",
      "role": "جایگاه این نرخ در اقتصاد ...",
      "gold_impact": "تاثیر بر طلا و توضیح ..."
    }}
  ],
  "summary": "جمع‌بندی کلی از داده‌های امروز و تاثیر آن بر طلا..."
}}
"""
